...
class TestUsers(TestCase):
    ...
    #     #                    'traceId': result[0].get('traceId')}, 400), result)
    #     self.assertEqual(({'errors': ["<class 'RuntimeError'>",
    #                                   'Working outside of application context.\n'
    #                                   '\n'
    #                                   'This typically means that you attempted to use functionality '
    #                                   'that needed\n'
    #                                   'the current application. To solve this, set up an application '
    #                                   'context\n'
    #                                   'with app.app_context(). See the documentation for more '
    #                                   'information.',
    #                                   '<traceback object at 0x000002264EEC22C0>'],
    #                        'traceId': result[0].get('traceId')}, 500), result)
    ...
    def test_create_user_with_username_fail(self, mock_request_parser, mock_generate_hash, mock_get_by_username):
        ...
        #                               'with app.app_context(). See the documentation for more '
        #                               'information.',
        #                               '<traceback object at 0x0000011D211C3740>'],
        #                    'traceId': result[0].get('traceId')}, 500), result)
    ...
    #     mock_current_user.return_value = 'username1'
    ...
    #
    #     self.get_user_json['username'] = 'username_new1'
    #
    #     self.assertEqual({'email': 'email@gmail.com',
    #                       'id': None,
    #                       'password': 'password',
    #                       'roles': ['admin'],
    #                       'username': 'username_new1'}, result)
    #
    # @mock.patch('src.model.user.User.save_to_db')
    # @mock.patch('src.model.user.User.get_by_id')
    # @mock.patch('src.model.user.Role.get_by_name')
    ...
    # @mock.patch('flask_httpauth.HTTPAuth.current_user')
    # @mock.patch('flask_restful.reqparse.RequestParser.parse_args')
    # def test_update_user_by_id_id_fail(self, mock_request_parser, mock_current_user, mock_get_by_username,
    #                                    mock_get_by_email, mock_get_by_name, mock_get_by_id, mock_save_to_db):
    #     mock_current_user.return_value = 'username1'
    #     mock_request_parser.return_value = self.update_user_json
    #     admin = Role(id=1, name='admin')
    ...
    #
    #     undecorated_update_user_by_id = undecorated(update_user_by_id)
    #     result = undecorated_update_user_by_id(9)
    #
    #     self.get_user_json['username'] = 'username_new1'
    #
    #     self.assertEqual(({'errors': [{'message': 'User with such id does not exist.',
